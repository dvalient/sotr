...
//				$texto = $e["leyenda"];
			...
			case 'variable':
				...
				if ($variable != NULL){
					...
					$color = imagecolorallocate($img, $e["ri"], $e["gi"], $e["bi"]);
					imagestring($img, $font, $x, $y, $valor_num.' '.$unidad, $color);
				}
			break;
		}
	}
	...
	$ancho = $pantalla[0]["w"];
	for ($i = 0; $i <= $ancho; $i += 10){
		imageline($img, $i, 1, $i, 3, $color);
		if(($i % 50)  == 0){
			imageline($img, $i, 1, $i, 7, $color);
		}
	}
	$alto = $pantalla[0]['h'];
	for ($i = 0; $i <= $alto; $i += 10){
		imageline($img, 1, $i, 3, $i, $color);
		if(($i % 50)  == 0){
			imageline($img, 1, $i, 7, $i, $color);
		}
	}
...
*/
	...
	$ancho =  imagesx($src);
	$alto = imagesy($src);
	imagecopymerge($img, $src, $pantalla[0]["w"]-$ancho, 1, 0, 0, $ancho, $alto, 50);
